...
{
    QFile segments("d:\\segments.txt");
    QTextStream in(&segments);
    if (!segments.open(QIODevice::ReadOnly)) {
...
void MainWindow::cut(int x1, int y1, int x2, int y2)
{
    ...
    cut((x1 + x2) / 2, (y1 + y2) / 2, x2, y2);
}

void MainWindow::ProcessPoly()
{
    QFile polygons("d:\\polygons.txt");
